---
title: "Advances in Sequential Monte Carlo Methods"
subtitle: "PhD Confirmation Seminar<br><br>Joshua J Bon<br><br>April 9 2020<br><br><br><br>"
author: ""
institute: ""
...
- Slides available at: https://bonstats.github.io/confirm

- Please save questions until of end presentation.


---
class: list-space, list-nobullet

## Confirmation of Candidature

- **Thesis Title**: "Advances in Sequential Monte Carlo Methods"

- **Principal supervisor**: A/Professor Christopher Drovandi

- **Associate supervisor**: Professor Tony Pettitt

- **External supervisor**: A/Professor Anthony Lee
...
2. Computational Bayesian inference

3. Sequential Monte Carlo

4. Motivation and aims

5. Research tasks and progress so far
...
- Mean: $\mathbb{E}_{p}[\boldsymbol{\theta}]$
- Variance: $\mathbb{E}_{p}[\boldsymbol{\theta}^2] - \mathbb{E}_{p}[\boldsymbol{\theta}]^2$
- Probabilities/Quantiles:  $\mathbb{E}_{p}[1(\boldsymbol{\theta} <q)]$

--

.full-width[.content-box-red[
Monte Carlo techniques allow approximate posterior expectations with simulations

$$\mathbb{E}_{p}[f(\boldsymbol{\theta})] \approx \frac{1}{N}\sum_{i=1}^{N}f(\boldsymbol{\theta}_{i}), \quad \boldsymbol{\theta}_{i} \sim p(\boldsymbol{\theta}~\vert~\boldsymbol{y})$$
]]
...
Construct a Markov chain whose invariant distribution is the target distribution.

--

$$\mathbb{E}_{p}[f(\boldsymbol{\theta})] \approx \frac{1}{N}\sum_{i=1}^{N}f(\boldsymbol{\theta}_{i}), \quad
(\boldsymbol{\theta}_{i}~\vert~\boldsymbol{\theta}_{i-1}) \sim K(\boldsymbol{\theta}_{i}~\vert~\boldsymbol{\theta}_{i-1})$$

--

Although samples are correlated, expectations can be approximated to a desired precision.
...
- A robust inferential tool
- Multimodality, intractability, dynamic models 
]
.content-box-red[

**But** there is still
- Less development than competitors
- Performance lags behind leading MCMC
]
]
...
**Performance** depends on its components 

- How well they adapt to a given problem
- The mutation step of SMC is crucial
]
.content-box-red[
Research on exploiting **SMC strengths**
...
`r emo::ji("notes")` **Better. Faster. Stronger.** `r emo::ji("notes")`
...
- Develop computationally efficient SMC algorithms
- Investigate and test new mutation kernels
- Exploit the population of particles to tune mutation kernels
]]
...
- S-SMC: Surrogate likelihoods in SMC
- K-SMC: Improving mutation kernels for SMC
- D-SMC: SMC with (approximate) likelihood gradients
- Theory: Theoretical considerations
]]
...
.full-width[.content-box-red[
**Aim**: Utilise a surrogate likelihood in SMC when the model's likelihood is expensive
 to compute]]
...
1. Tune delayed-acceptance kernel by optimising computation time

--
2. Calibration surrogate likelihoods within SMC

--
3. Surrogate first annealing strategy
...
.full-width[.content-box-red[
- MCMC method generates correlated posterior samples
- No need for normalising constant (in most cases)
]]
...
2. Calculate **acceptance probability** $$\alpha = \min\left(1,\frac{p(\boldsymbol{\theta}_{t}^{\prime}~\vert~\boldsymbol{y})q(\boldsymbol{\theta}_{t-1}~\vert~\boldsymbol{\theta}_{t}^{\prime})}{p(\boldsymbol{\theta}_{t-1}~\vert~\boldsymbol{y})q(\boldsymbol{\theta}_{t}^{\prime}~\vert~\boldsymbol{\theta}_{t-1})} \right)$$

--

3. **Evaluate** proposal

  (a) Reject: $\boldsymbol{\theta}_{t} \leftarrow \boldsymbol{\theta}_{t-1}$ with probability $1 - \alpha$, or
  
  (b) Accept: $\boldsymbol{\theta}_{t} \leftarrow \boldsymbol{\theta}_{t}^{\prime}$ with probability $\alpha$

--

Generally we use several MH cycles to diversify particles
...
## Background: Adaptive delayed-acceptance in SMC
...
## Background: Diversification criteria
...
## Contribution: Optimising computation time
...
## Contribution: Calibrating the surrogate likelihood

.full-width[.content-box-red[
**Key idea**: Use location history of the particles to calibrate the surrogate likelihood (better match the full likelihood).
]]
...
- $d$ is some discrepancy measure, e.g. squares difference of the log-likelihoods
...
---
## Contribution: Surrogate first annealing
...
## S-SMC Results: Surrogate evaluations
...
## S-SMC Results: Example posterior comparisons
...
1. `r emo::ji("check")` Conduct a further literature review of delayed-acceptance methods in MCMC, SMC, and ABC-SMC.
2. `r emo::ji("check")` Write `R` package to implement generic SMC algorithms.
3. `r emo::ji("check")` Write `R` and `C++` code to fit models often used to test delayed-acceptance-type algorithms. 
4. `r emo::ji("check")` Identify potential surrogate posteriors with possible tuning parameters, and tuning mechanism(s).
5. `r emo::ji("check")` Test these surrogates and tuning mechanism(s) across appropriate testing models.
6. `r emo::ji("construction")` Apply methods to computationally expensive time series models 
7. `r emo::ji("construction")` Finish and submit paper "Delayed-acceptance sequential Monte Carlo"
...
.full-width[.content-box-red[
**Aim**: Use mutation kernels in SMC that preserve the relative probability of the particle locations.
]]
...
--
1. Investigate location-dependent kernel mixtures. In particular, extend independent-copula proposals [(South et al, 2019)](https://doi.org/10.1214/18-BA1129) with location dependence to better explore the posterior tails.

--
2. Investigate the mirror kernel for SMC.

--
3. Investigate extensions of the mirror kernel for SMC.
...
1. `r emo::ji("construction")` Further literature review of mixed-kernel methods in MCMC.
2. `r emo::ji("construction")` Investigate mirror kernel for SMC.
3. Detailed study of mixed-kernels in MCMC and SMC.
4. `r emo::ji("construction")` Identify groups of kernels that complement their counterparts' weaknesses. 
5. Write `R` code to implement SMC with mixtures (or cycles) of the identified groups of kernels.
6. Identify then test specific applications where the mixed-kernels may better explore tails of the posterior.
7. Write and submit paper "Locally-adaptive mutation kernels in sequential Monte Carlo"

---
class: inverse, middle, center, hide-logo

# D-SMC: (Approximate) likelihood gradients
...
.full-width[.content-box-red[
**Aim**: Use approximate derivative information in SMC to guide mutation step.
]]
...
--
1. Investigate current shortcomings of adaptive SMC mutation kernels using (approximate) derivative information

--
2. Identify potential candidates for efficient (approximate) derivative-based mutation kernels. 

--
3. Test these kernels, including an approximate version of the Hug and Hop kernel [(Ludkin & Sherlock, 2019)](https://arxiv.org/abs/1907.13570)
...
## D-SMC tasks
 ...
 1. `r emo::ji("construction")` Further literature review of methods which use or could use approximate derivatives in MCMC and SMC.
 2. `r emo::ji("construction")`  Identify or propose potential methods, e.g. approximate Hug and Hop kernel.
 3. Write `R` code to implement methods.
 4. Identify then test specific applications where these algorithms may reduce computation time or enable computation.
 5. Write and submit paper "Efficient and derivative-free posterior analysis with sequential Monte Carlo".
...
.full-width[.content-box-red[
**Aim**: Expand theoretical results on properties of adaptive SMC, particularly those using adaptive mutation kernels.
]]
...
--
1. Investigate if guarantees available in the literature are applicable to SMC algorithm with adaptive mirror kernel. If not, try to extend these [(e.g. Beskos et al, 2016)](http://dx.doi.org/10.1214/15-AAP1113)  

--
2. Investigate if guarantees are applicable to SMC with other adaptive mutation kernels used in the previous streams. If not, try to extend these guarantees.
...
1. `r emo::ji("construction")` Literature review of current theoretical guarantees afforded to adaptive SMC algorithms.
2. Identify adaptive SMC algorithms that are applicable or where progress is possible.
3. Develop theory for adaptive SMC algorithms. Use SMC tuned mirror kernel as starting place.
4. Write and submit paper "Guarantees for new adaptive Sequential Monte Carlo algorithms".
...
```{r pdf-convert, echo = F, eval = F}

for(i in  1:3){
pdftools::pdf_convert(paste0("imgs/latex-files/timeline-",i,"-1.pdf"), format = "png", dpi = 72, filenames = paste0("imgs/timeline-",i,"-1.png"))
}

```

```{r timeline-1, echo=FALSE, eval=TRUE,  out.width = '95%'}
knitr::include_graphics("imgs/timeline-1-1.png")
...
```{r timeline-2, echo=FALSE, eval=TRUE,  out.width = '95%'}
knitr::include_graphics("imgs/timeline-2-1.png")
...
```{r timeline-3, echo=FALSE, eval=TRUE,  out.width = '95%'}
knitr::include_graphics("imgs/timeline-3-1.png")
...
- A/Professor Christopher Drovandi
...
- A/Professor Anthony Lee

--

Heartfelt thanks must also go to:

- School of Mathematical Science at QUT

- ACEMS at QUT

- ACEMS at other nodes
